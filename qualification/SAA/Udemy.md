...
## EC2（1:48,63）（2:8） [2:5,40] [3:1,10,18,36] [4:1,30]
...
## VPC （2:36,48）[1:58] [2:34] [3:34] [4:4,16]
  * [X] ルートテーブル
...
## EBS（1:8）（2:11）[1:19] [2:10,33] [3:6,13,45]
  * EBSレプリケーションという機能は無い
...
## DynamoDB (1:9) (2:12,65) [3:23,39] [4:17,48]
  * JSON形式のファイルを保存
...
## S3（1:12,24,30,36,61）（2:13,14,15,18,25,40,41,49）[1:4,37,49,64] [2:14,21,36,50] [3:33,64] [4:7,24] [5:2,3]
...
## FSx for Windows File Server [2:39] [4:46]
...
## RDS（1:10,32）[1:14,16] [2:43] [3:32,35] [4:8]
  * RDS Proxy
...
## CloudFront（1:3,22,33）（2:25） [2:46,52] [4:23,60,61] [5:5]
  ...
  * 料金は、AWSからのデータ転送と、顧客へのコンテンツ配信に使用されるリクエスト
  * 
...
## Auto Scaling（1:45）（2:45）[1:3,19] [2:40] [3:50]
  * 設定フロー
...
## RedShift（1:41）[1:1] [2:44] [3:27] [4:47,52]
