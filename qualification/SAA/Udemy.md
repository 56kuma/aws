...
## わからん言葉
  ...
  * [X] NATインスタンスと、NATゲートウェイの違い
    ...
    * NATゲートウェイ
      ...
      * 作成後は、"プライベートサブネット"のルートテーブルを更新して、インターネットトラフィックをNATゲートウェイに向かわせる
  ...
  * [X] Cloud HSM（Hardware Security Module）
    * クラウドベースのハードウェアセキュリティモジュール
    * FIPS 140-2のレベル3認証済のHSM
    * ゼロ化してキーをロスしてしまうと、コピーを有していない場合、新しいキーは取得不可能
