...
## ELB
  ...
  * EC2ヘルスチェック
    * デフォルトで有効
    * 以下の状態である場合、EC2 Auto Scalingはインスタンスを異常であるとみなす
      * stopping
      * stopped
      * shutting-down
      * terminated
  * ELBヘルスチェック
    * 正常なインスタンスのみをELBに追加
    * OutOfServiceとレポートしたときにインスタンスを異常とマークする
...
## VPC
  ...
  * VPC作成時に割り当てるCIDRブロックがプライマリCIDR、後から追加するのがセカンダリCIDR
